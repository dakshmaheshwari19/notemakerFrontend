import {
  ...
  loadNotesFailure,
  updateNote,
  changeNoteStatePending,
  changeNoteStateSuccess,
  changeNoteStateFailure,
  deleteNote,
  addNote,
} from "../../feature/Notes/notesReducer.js";
...
export const loadNotes = () => async (dispatch) => {
  try {
    ...
    const response = await axios.post(
      `${import.meta.env.VITE_SERVER}/notes/loadNotes`,
      {},
...
export const changeNoteState =
  (noteId, starred, archived, trashed) => async (dispatch) => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_SERVER}/notes/changeState`,
        {
          noteId: noteId,
          starred: starred,
          archived: archived,
...
export const deleteNoteById = (noteId) => async (dispatch) => {
  try {
    dispatch(changeNoteStatePending()); // Dispatch action to indicate pending state
    const response = await axios.post(
      `${import.meta.env.VITE_SERVER}/notes/deleteNote`,
      {
        noteId: noteId,
      }
    );
...
export const addNewNote = (title, description) => async (dispatch) => {
  try {
    dispatch(changeNoteStatePending()); // Dispatch action to indicate pending state
    const response = await axios.post(
      `${import.meta.env.VITE_SERVER}/notes/addNote`,
      {
        title: title,
...
export const editNote = (noteId, title, description) => async (dispatch) => {
  try {
    dispatch(changeNoteStatePending()); // Dispatch action to indicate pending state
    const response = await axios.post(
      `${import.meta.env.VITE_SERVER}/notes/editNote`,
      {
        noteId: noteId,
        title: title,
        description: description,
